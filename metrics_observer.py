...
Ported from gemini-s2s with one correctness fix:
- TTFB attribution now checks "tts" BEFORE vendor keywords, so
  GeminiTTSService TTFB is attributed to TTS, not LLM. The original
  matcher fell into "gemini" → LLM first, which would have invalidated
  Agent B's pipeline waterfall.
"""
...
def _classify_processor(proc: str) -> str | None:
    """Classify a processor name into one of stt / llm / tts, or None.

    Order matters: check tts/stt keywords BEFORE vendor keywords, because
    a vendor name like "gemini" appears in both the LLM service and the
    TTS service. Without this order, GeminiTTSService TTFB would be
    misattributed to LLM.
    """
    p = proc.lower()

    # TTS first — any service with "tts" in the name is a TTS service
    if "tts" in p or "elevenlabs" in p or "cartesia" in p:
        return "tts"

    # STT second
    if "stt" in p or "deepgram" in p or "parakeet" in p or "nvidia" in p:
        return "stt"

    # LLM last — vendor keywords only land here once tts/stt are ruled out
    if "llm" in p or "gemini" in p or "google" in p or "anthropic" in p or "openai" in p:
        return "llm"
...
class MetricsCollectorObserver(BaseObserver):
    ...
    async def on_push_frame(self, data: FramePushed):
        # Pipecat 1.0.0 pushes BotStartedSpeakingFrame / BotStoppedSpeakingFrame
        # upstream from the transport's output layer, so we can't filter by
        # direction. Instead we dedup by frame ID — each frame has a unique
        # ID regardless of direction, so we'll only process it once.
    ...
    def _on_metrics(self, frame: MetricsFrame):
        for m in frame.data:
            if isinstance(m, SmartTurnMetricsData):
                ...
                kind = _classify_processor(m.processor)
                if kind == "stt":
                    self._stt_ttfb = m.value
                elif kind == "llm":
                    self._llm_ttfb = m.value
                elif kind == "tts":
                    self._tts_ttfb = m.value
            elif isinstance(m, ProcessingMetricsData):
                if _classify_processor(m.processor) == "stt":
